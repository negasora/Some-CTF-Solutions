...
from pwn import remote, unhex


r = remote('challs.pwnoh.io', 13419)

r.recvuntil(b'Signature:\n')
next_key = unhex(r.recvline().strip())



msg_hex = (my_message + b'______french fry').hex()
sig_hex = aes(b'french fry______', next_key).hex()

r.sendlineafter(b'>', msg_hex)
r.sendlineafter(b'>', sig_hex)
